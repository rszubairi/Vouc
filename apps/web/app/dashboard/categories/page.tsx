...
export default function CategoriesPage() {
  ...
  const categories = useQuery(api.categories.list, { includeDeleted: showDeleted });
  const divisions = useQuery(api.divisions.list, {});
  const create = useMutation(api.categories.create);
  const update = useMutation(api.categories.update);
  const remove = useMutation(api.categories.remove);
  const bulkRemove = useMutation(api.categories.bulkRemove);
  const bulkRestore = useMutation(api.categories.bulkRestore);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkRemove({ ids: Array.from(selectedIds) as Id<"categories">[] });
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      await bulkRestore({ ids: Array.from(selectedIds) as Id<"categories">[] });
      setSelectedIds(new Set());
    } finally {
