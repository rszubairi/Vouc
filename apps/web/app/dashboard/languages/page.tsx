...
export default function LanguagesPage() {
  ...
  const languages = useQuery(api.languages.list, { includeDeleted: showDeleted });
  ...
  const bulkRestore = useMutation(api.languages.bulkRestore);
  ...
  const visibleLanguages = showDeleted
    ? (languages ?? []).filter((l) => l.isDeleted)
    : languages;
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      await bulkRestore({ ids: Array.from(selectedIds) as Id<"languages">[] });
      setSelectedIds(new Set());
    } finally {
