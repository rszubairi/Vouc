...
export default function MarketsPage() {
  ...
  const markets = useQuery(api.markets.list, { includeDeleted: showDeleted });
  ...
  const bulkRestore = useMutation(api.markets.bulkRestore);
  ...
  const visibleMarkets = showDeleted ? (markets ?? []).filter((m) => m.isDeleted) : markets;
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      await bulkRestore({ ids: Array.from(selectedIds) as Id<"markets">[] });
      setSelectedIds(new Set());
    } finally {
